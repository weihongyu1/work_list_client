<template>
  ...
  <div style="width: 50%;border-right: 1px solid #f4f4f5;float: left;">
    ...
    <div style="margin-top: 20px">
      <input v-model="task.task" class="input-task" type="text" placeholder="+ 添加学习安排"/>
      <el-date-picker
          v-model="task.date"
          type="datetime"
      ...
      <!--      分类-->
      <span class="el-dropdown-link" style="color: #ff6681;cursor: pointer;margin-left: 5px">学习安排</span>

      <img alt="未加载" src="../../../static/img/icon/add.png" style="height: 15px;margin-left: 5px" @click="addTask">
    </div>
    ...
    <div>
      <ul>
        <li v-for="(item,index) in tasks" class="task" @click="listDetails(item)">
          ...
          <img alt="未加载" src="../../../static/img/icon/delete.png" class="delete-task" style="height: 15px;margin-left: 30px;" @click="deleteTask(item)">
        </li>
      </ul>
    </div>
  </div>
